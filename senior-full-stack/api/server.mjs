...
app.use((_, response, next) => { response.header('Access-Control-Allow-Origin', '*'); next() })
app.get('/health', (_, response) => response.json({ status: 'ok', runtime: 'python-pyodide' }))
app.get('/api/services', (_, response) => response.json(services.map(({ delay_ms, ...service }) => service)))
app.get('/api/services/:serviceId/dashboard', async (request, response, next) => {
  try {
