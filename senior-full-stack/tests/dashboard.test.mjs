...
const load = async (name) =>
  JSON.parse(
    await readFile(new URL(`../api/data/${name}`, import.meta.url), 'utf8'),
  )
...
test('catalog dashboard returns its service and incidents', async () => {
  ...
  assert.deepEqual(
    dashboard.incidents.map((incident) => incident.id),
    ['INC-404'],
  )
})
...
test('summary is derived from the returned incident set', async () => {
  ...
  assert.equal(
    dashboard.summary.active_incidents,
    dashboard.incidents.filter((incident) => incident.status === 'open').length,
  )
